...
`RowPartitionFullRow` count/shape 注意事项：

- 对 `tensor[idx][{}]` / `tensor[{}][idx]` 这类 full payload 输入，本地缓存应按
  **unique indexed value** 存一份 payload，而不是按每个 output item 重复 payload。
- 对 2D row-partition ownership，indexed row payload 可按 owned output rows
  `Scatterv`；indexed column payload 若所有 row-partition rank 都会计算所有输出列，
  应 broadcast 完整 column payload 集合给所有 rank。
- MPI `Scatterv` / `Gatherv` 的 count/displ 仍是 `int`，所有由 shape 推导出的
  element count / byte count 都必须在生成代码中用 `int64_t` 计算并在转 `int`
  前做上界检查。`matmul2048` 暴露过旧实现把
  `output_item_count * full_payload_len` 当作 per-rank input count，导致
  `512*2048*2048 = 2,147,483,648` 超过 MPI `int` 上限。
...
| Phase 3 | 已完成 | `ReplicatedFullTensor`、`RowPartitionFullRow`、受限 `READ_WRITE OutputDirect` | `DFT1.0` `gradientSum` `jacobi1.0` `matMul1.0` | 更广的 full-payload 形态仍未展开；large full-payload benchmark 需继续守住 MPI count/shape 安全 |
...
| Phase 4.6 | 已完成 | loop-lowered stencil full-sync skeleton、安全 reader hoist、RowPartitionFullRow count 修复、1D `StencilWindow1D` `+1` resident halo、2D `StencilWindow2D` row-block resident halo、`waveEquation1.0` 所需 direct-reader / `-1,-1` read-cache resident path | `MDP1.0` `liuliang1.0` `stencil1.0` `waveEquation1.0` `mpiLoopStencilResidentHalo1D` `mpiLoopStencilResidentHaloEmptyRank1D` `mpiLoopStencilRightBoundaryFullSync1D` `mpiLoopStencilScalarReject2D` `mpiLoopStencilCountGuard2D` `mpiLoopStencilOrderReject2D` | 更广 direct-reader/cache、root-bridge、dynamic shape 和 Phase 5 `FixedBlock` 仍留在后续阶段 |
...
当前基线结果已经随 P4.6 增量扩展；截至 2026-05-09 的关键回归结果包括：

- P4.6 review 修复后：
  - `cmake --build build --target translator -j8` 通过。
  - `bash test_mpi.sh mpiLoopStencilResidentHaloEmptyRank1D mpiLoopStencilRightBoundaryFullSync1D` 通过 2/2。
  - `bash test_mpi.sh mpiLoopStencilInvariantReader1D mpiLoopStencilAliasRefresh1D mpiLoopStencilReferenceAliasRefresh1D mpiLoopStencilResidentHalo1D` 通过 4/4。
  - `bash test_mpi.sh MDP1.0 liuliang1.0 stencil1.0 waveEquation1.0` 通过 4/4。
...
2026-05-09 large benchmark 观察：

- `matmul2048` 是 `RowPartitionFullRow` 的 count/shape 压测。正确布局应复用 unique
  row/column payload；修复后 4 rank 样本输出为 `147385 147273 147961`，
  `/usr/bin/time -p` wall time 约 0.31-0.34s。
- `waveEquation1.0` large (`NX=2048, NY=2048, TIME_STEPS=500`) 在 Route A
  full-sync skeleton 下 300s 内未完成，且 stdout 仍为 0B，说明瓶颈发生在循环内，
  不是最终 `matCur.print()`。每步仍有 full reader bcast、direct-reader bcast、
  writer gatherv、read-cache full replay+bcast、followup/boundary full replay+bcast
  和多次 host `tensor2Array` / `array2Tensor`。这正是 Route B resident halo /
  resident read-cache 要解决的性能债。

2026-05-09 P4.6 当前进度：

- Route A 已完成 loop-lowered full-sync skeleton 和安全 reader hoist。无法证明
  reader loop-invariant、reader/writer alias 或 reference alias 时，full reader sync
  保持在 `run()`。
- `RowPartitionFullRow` 已修复 large count/shape 风险：full row/column payload 按
  unique indexed row/column 存储，避免 `matmul2048` 的 `MPI_ERR_COUNT`。
- Route B B1 已完成当前首切片：`StencilWindow1D`、一个 window reader、一个
  WRITE-only direct writer、`+1` distributed followup、字面左边界 `0`
  boundary-local update。无法证明的 boundary / coverage 继续走 Route A full-sync
  fallback。
- B1 review 边界已修复：空 owned-output rank 不参与实际 payload 通信，但有数据
  rank 会与最近非空 rank 匹配 halo exchange；右边界或符号边界不会误走 B1。
- Route B B2 已完成：`StencilWindow2D` row-block resident halo 已覆盖
  `stencil1.0` 所需的一个 2D window reader、一个 WRITE-only direct writer、当前
  `+1,+1` distributed followup 与当前 boundary-local updates，并补齐了 scalar
  reject 与 2D count/displ/materialize/layout overflow guard。
- Route B B3 已完成：`StencilWindow2D` resident halo 在 B2 基础上保守加入
  `waveEquation1.0` 当前所需的一个 2D direct reader、`matCur -> matPrev`
  `(-1,-1)` read-cache transition、direct-reader resident state，以及当前四条零值
  boundary-local updates，以及当前 `DAC -> read-cache -> followup -> boundary`
  顶层语句顺序。无法证明 distinct tensor key、coverage、boundary、当前语句顺序或
  host-visible tensor 语义时，仍继续走 Route A `StencilFullSync` fallback。

2026-05-09 按 `docs/benchmarks` 的大规模口径重跑筛选后的 app benchmark
（筛掉尚未进入 B2/B3 的复杂 stencil）：
...
只统计 `mpirun` 运行阶段外部 wall-clock，不包含翻译/编译，不列串行 baseline。
结果文件为
`/Volumes/QUQ/working/mpi_tmp/p46_b1_selected_large_20260509/results.tsv`。

| Test | Data size | Hand-written MPI+SYCL s | DAC translated MPI s |
|---|---:|---:|---:|
| `DFT1.0` | `N=4096` | 0.877217 | 0.888717 |
| `MDP1.0` | `N=8192, T=600` | 0.827930 | 0.828125 |
| `decay1.0` | `numIsotopes=8192, steps=600` | 1.108226 | 0.891583 |
| `gradientSum` | `8192x4096` | 0.662446 | 1.074782 |
| `imageAdjustment1.0` | `4096x4096` | 0.717041 | 0.892109 |
| `jacobi1.0` | `N=4096, iter=300` | 0.737592 | 0.877352 |
| `liuliang1.0` | `WIDTH=8192, steps=1000` | 0.937474 | 0.840554 |
| `mandel1.0` | `4096x4096, max_iter=1000` | 2.535479 | 3.029412 |
| `matMul1.0` | `2048x2048` | 5.921520 | 0.902443 |
| `oddeven0.1` | `N=4096` | 1.895137 | 5.617574 |
| `vectorAddCombo` | `N=8388608` | 0.824343 | 0.749346 |

推荐拆分顺序：

1. 先做 measurement-only benchmark，把旧实现、新实现和手写 MPI+SYCL 的通信次数与运行时间口径固定。
2. 再做 2D resident state 角色轮转，而不是继续扩大 stencil 语义：
   - `stencil1.0` 先把 `apply_followup_2d(...)` 变成双缓冲 swap
   - `waveEquation1.0` 再把 `prev/cur/next` 变成三缓冲轮转，替掉当前
     `apply_read_cache_transition_2d(...)` + `apply_followup_2d(...)` 的 slab 复制
3. 再做 1D / loop-site gate 收窄与补接，例如让 `FOuLa1.0` 进入现有 P4.6 skeleton /
   resident family，而不是停在 `not inside a stable loop site`
4. P4.6 已在 B3 收口：当前 2D stencil resident cache/direct-reader 只覆盖
   `waveEquation1.0` 所需 window reader + direct reader + writer + `-1,-1`
   read-cache transition；更广的 direct-reader/cache 形态继续保守 fallback。
5. `oddeven0.1` 的明显性能问题对应的是 Phase 5 `FixedBlock`，不应混入 P4.6。
