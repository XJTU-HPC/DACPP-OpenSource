...
| Phase 4.6 | Route A + Route B B1/B2 已落地，B3 待完成 | loop-lowered stencil full-sync skeleton、安全 reader hoist、RowPartitionFullRow count 修复、1D `StencilWindow1D` `+1` resident halo、2D `StencilWindow2D` row-block resident halo | `MDP1.0` `liuliang1.0` `stencil1.0` `mpiLoopStencilResidentHalo1D` `mpiLoopStencilResidentHaloEmptyRank1D` `mpiLoopStencilRightBoundaryFullSync1D` `mpiLoopStencilScalarReject2D` `mpiLoopStencilCountGuard2D` | B3 `waveEquation1.0` direct-reader / read-cache resident path 尚未落地；`waveEquation1.0` 仍依赖 Route A full-sync fallback |
...
- 下一步是 B3：`waveEquation1.0` 所需的 direct reader / `-1,-1` read-cache
  transition resident path。无法证明 resident cache coverage、boundary 或 host-visible
  tensor 语义时，仍继续走 Route A full-sync fallback。
...
4. B2 已完成后，下一步做 2D stencil resident cache/direct-reader（B3），只覆盖
