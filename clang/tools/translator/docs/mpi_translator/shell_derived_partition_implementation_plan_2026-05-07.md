...
| Phase 4.6 | 已开始，Route A + B1 首切片已落地 | loop-lowered stencil full-sync skeleton、安全 reader hoist、RowPartitionFullRow count 修复、1D `StencilWindow1D` `+1` resident halo | `MDP1.0` `liuliang1.0` `mpiLoopStencilResidentHalo1D` `mpiLoopStencilResidentHaloEmptyRank1D` `mpiLoopStencilRightBoundaryFullSync1D` | B2 2D row-block resident halo 尚未落地；`stencil1.0` / `waveEquation1.0` 仍依赖 full-sync fallback |
...
- 下一步是 B2：`StencilWindow2D` row-block resident halo for `stencil1.0`，不包含
  direct reader / read-cache transition。`waveEquation1.0` 属于 B3。
...
4. B1 已完成后，下一步先做 2D stencil row-block resident halo（B2），只覆盖
   `stencil1.0` 所需 window reader + writer + `+1,+1` followup；direct reader /
   read-cache transition 留给 B3。
