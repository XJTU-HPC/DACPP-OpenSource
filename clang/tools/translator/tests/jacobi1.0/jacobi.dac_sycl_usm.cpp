...
void jacobiShell_jacobi(const dacpp::Matrix<float> & A, const dacpp::Vector<float> & b, const dacpp::Vector<float> & x, dacpp::Vector<float> & x_new, const dacpp::Vector<int> & nums) { 
	...
	A.tensor2Array(h_A);
    q.memcpy(d_A,h_A,A_Size*sizeof(float)).wait();

    // 数据移动
	float* h_b = (float*)malloc(b_Size*sizeof(float));
	b.tensor2Array(h_b);
    q.memcpy(d_b,h_b,b_Size*sizeof(float)).wait();

    // 数据移动
	float* h_x = (float*)malloc(x_Size*sizeof(float));
	x.tensor2Array(h_x);
    q.memcpy(d_x,h_x,x_Size*sizeof(float)).wait();

    // 数据移动
	float* h_x_new = (float*)malloc(x_new_Size*sizeof(float));
	// x_new.tensor2Array(h_x_new);
    q.memset(d_x_new, 0, x_new_Size*sizeof(float)).wait();
    // 数据移动
	int* h_nums = (int*)malloc(nums_Size*sizeof(int));
	nums.tensor2Array(h_nums);
    q.memcpy(d_nums,h_nums,nums_Size*sizeof(int)).wait();
	...
	float *r_A=malloc_device<float>(A_Size,q);
	...
	float *r_b=malloc_device<float>(b_Size,q);
	...
	float *r_x=malloc_device<float>(x_Size,q);
    x_tool.Reconstruct(r_x,d_x,q);
	...
	float *r_x_new=malloc_device<float>(x_new_Size,q);
    x_new_tool.Reconstruct(r_x_new,d_x_new,q);
	...
	int *r_nums=malloc_device<int>(nums_Size,q);
    nums_tool.Reconstruct(r_nums,d_nums,q);
    ...
    q.submit([&](handler &h) {
        ...
        h.parallel_for(sycl::nd_range<3>(global, local),[=](sycl::nd_item<3> item) {
            ...
            jacobi(r_A+(idx1_*SplitLength[0][0]),r_b+(idx1_*SplitLength[1][0]),r_x,r_x_new+(idx1_*SplitLength[3][0]),r_nums+(idx1_*SplitLength[4][0]),info_partition_A_accessor,info_partition_b_accessor,info_partition_x_accessor,info_partition_x_new_accessor,info_partition_nums_accessor);
        });
    }).wait();
        ...
        for(int i=0;i<x_newReduction_Size;i++) {
            q.submit([&](handler &h) {
                ...
                [=](id<1> idx,auto &reducer) {
                    reducer.combine(r_x_new[(i/Reduction_Split_Length)*Reduction_Split_Length*Reduction_Split_Size+i%Reduction_Split_Length+idx*Reduction_Split_Length]);
     	        });
         }).wait();
        }
        q.memcpy(r_x_new,reduction_x_new, x_newReduction_Size*sizeof(float)).wait();
    }


    // 归并结果返回
    x_new_tool.UpdateData(r_x_new,d_x_new,q);
	q.memcpy(h_x_new,d_x_new, x_new_Size*sizeof(float)).wait();
