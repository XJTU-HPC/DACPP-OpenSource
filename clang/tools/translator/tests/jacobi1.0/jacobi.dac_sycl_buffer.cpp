...
void jacobiShell_jacobi(const dacpp::Matrix<float> & A, const dacpp::Vector<float> & b, const dacpp::Vector<float> & x, dacpp::Vector<float> & x_new, const dacpp::Vector<int> & nums) { 
    ...
    // Buffer设备内存分配
    buffer <float> b_A{A_Size};

    // Buffer设备内存分配
    buffer <float> b_b{b_Size};

    // Buffer设备内存分配
    buffer <float> b_x{x_Size};

    // Buffer设备内存分配
    buffer <float> b_x_new{x_new_Size};

    // 规约Buffer设备内存分配
    std::vector<sycl::buffer<float, 1>> b_reduction_x_new(x_newReduction_Size, buffer<float, 1>{1});
    for(int i = 0; i < x_newReduction_Size; i++){
        host_accessor temp_accessor{b_reduction_x_new[i]};
        temp_accessor[0] = 0;
    }
    // Buffer设备内存分配
    buffer <int> b_nums{nums_Size};
    ...
    A.tensor2Array(h_A);
    {
        host_accessor temp_accessor{b_A};
        for(int i = 0; i < A_Size; i++){
            temp_accessor[i] = h_A[i];
        }
    }
    ...
    b.tensor2Array(h_b);
    {
        host_accessor temp_accessor{b_b};
        for(int i = 0; i < b_Size; i++){
            temp_accessor[i] = h_b[i];
        }
    }
    ...
    {
        host_accessor temp_accessor{b_nums};
        for(int i = 0; i < nums_Size; i++){
            temp_accessor[i] = h_nums[i];
        }
    }
    ...
    buffer<float> r_A{A_Size};
    A_tool.Reconstruct(r_A,b_A,q);
    ...
    buffer<float> r_b{b_Size};
    b_tool.Reconstruct(r_b,b_b,q);
    ...
    buffer<float> r_x{x_Size};
    x_tool.Reconstruct(r_x,b_x,q);
    ...
    buffer<float> r_x_new{x_new_Size};
    x_new_tool.Reconstruct(r_x_new,b_x_new,q);
    ...
    buffer<int> r_nums{nums_Size};
    nums_tool.Reconstruct(r_nums,b_nums,q);
    ...
    q.submit([&](handler &h) {
        ...
        accessor acc_A{r_A, h};
        accessor acc_b{r_b, h};
        accessor acc_x{r_x, h};
        accessor acc_x_new{r_x_new, h};
        accessor acc_nums{r_nums, h};
    ...
    if(Reduction_Split_Size > 1)
    {
        for(int i=0;i<x_newReduction_Size;i++) {
            q.submit([&](handler &h) {
                accessor d_x_new{r_x_new, h};
    	        h.parallel_for(
                range<1>(Reduction_Split_Size),
                reduction(b_reduction_x_new[i], h, 
    ...
    //结果返回
    x_new_tool.UpdateData(r_x_new,b_x_new,q);
    {
        host_accessor temp_accessor{b_x_new};
        for(int i = 0; i < x_new_Size; i++){
            h_x_new[i] = temp_accessor[i];
        }
    }
