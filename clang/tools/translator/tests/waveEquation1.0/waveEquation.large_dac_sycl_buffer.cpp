...
inline void print_e2e_summary(int mpi_rank, double total_time) {
    ...
    if (mpi_rank == 0) {
        std::cerr << "[MPI_DAC][wave] e2e_seconds=" << total_max << std::endl;
    }
}
}  // namespace __dacpp_test
    ...
    for (int __or_block_step = 0; __or_block_step < __or_inner_steps; ++__or_block_step) {
        ...
        if (__or_kernel_item_count > 0) {
            ...
            q.submit([&](sycl::handler& h) {
                ...
                h.parallel_for<class __dacpp_sycl_kernel_waveEquation_row_temporal_0>(sycl::range<1>(static_cast<std::size_t>(__or_kernel_item_count)), [=](sycl::id<1> idx) {
...
for(int i = 0;false; i++) {
    ...
    if (__dacpp_mpi_is_root_rank()) {
        // Full matrix printing is intentionally disabled for Sunway benchmark runs.
    }
