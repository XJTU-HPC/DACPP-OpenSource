...
inline void print_summary(const char* label, int mpi_rank, int mpi_size, double total_time) {
    ...
    if (mpi_rank == 0) {
        std::printf(
            ...
            communication_max,
            communication_sum / static_cast<double>(mpi_size),
            computation_max,
            computation_sum / static_cast<double>(mpi_size));
    }
}
}  // namespace __dacpp_test
...
void __dacpp_mpi_or_waveEqShell_waveEq_0_init(__dacpp_mpi_or_waveEqShell_waveEq_0_ctx& ctx, dacpp::Matrix<double> & __or_arg0, dacpp::Matrix<double> & __or_arg1, dacpp::Matrix<double> & __or_arg2) {
    ...
    ctx.__or_spatial_layout = dacpp::mpi::operator_resident::resident_halo_2d_spatial_layout(ctx.__or_output_rows, ctx.__or_output_cols, ctx.__or_input_rows, ctx.__or_input_cols, ctx.mpi_rank, ctx.mpi_size, ctx.__or_spatial_halo_width);
    ctx.__or_output_row_range = ctx.__or_spatial_layout.owned_rows;
    ctx.__or_output_col_range = ctx.__or_spatial_layout.owned_cols;
    ctx.__or_local_output_rows = ctx.__or_output_row_range.count;
    ctx.__or_local_output_cols = ctx.__or_output_col_range.count;
    ctx.__or_output_row_begin = ctx.__or_output_row_range.begin;
    ctx.__or_output_col_begin = ctx.__or_output_col_range.begin;
    ...
    ctx.__or_halo_layout = dacpp::mpi::operator_resident::resident_halo_2d_row_layout(0, ctx.__or_input_cols, ctx.mpi_rank, ctx.mpi_size, ctx.__or_window_rows);
    ...
    dacpp::mpi::operator_resident::narrow_mpi_count_or_abort(static_cast<int64_t>(ctx.__or_spatial_layout.local_size), "[DACPP][MPI][OR] spatial resident halo 2D local reader size exceeds MPI int range");
    ...
    const int64_t __or_local_reader_size = ctx.__or_spatial_layout.local_size;
    ...
    }
    {
        ...
        dacpp::mpi::operator_resident::scatter_window_2d_spatial(__or_initial_global_window_prev, ctx.__or_local_prev, ctx.__or_output_rows, ctx.__or_output_cols, ctx.__or_input_rows, ctx.__or_input_cols, ctx.__or_spatial_halo_width, ctx.__or_spatial_layout, ctx.mpi_rank, ctx.mpi_size, MPI_DOUBLE);
    }
...
void __dacpp_mpi_or_waveEqShell_waveEq_0_run(__dacpp_mpi_or_waveEqShell_waveEq_0_ctx& ctx, dacpp::Matrix<double> & __or_arg0, dacpp::Matrix<double> & __or_arg1, dacpp::Matrix<double> & __or_arg2) {
    ...
    const int64_t __or_local_reader_rows = ctx.__or_spatial_layout.local_row_count;
    const int64_t __or_local_reader_cols = ctx.__or_spatial_layout.local_col_count;
    const int64_t __or_local_reader_row_begin = ctx.__or_spatial_layout.global_row_begin;
    const int64_t __or_local_reader_col_begin = ctx.__or_spatial_layout.global_col_begin;
    ...
    const int __or_inner_steps = static_cast<int>(std::min<int64_t>(static_cast<int64_t>(ctx.__or_temporal_block_size), __or_time_end - __or_time_step));
    auto dacpp_profile_halo_start = dacpp::mpi::profileSegmentStart();
    {
        __dacpp_test::ScopedCommunicationTimer __dacpp_test_comm_timer;
        dacpp::mpi::operator_resident::exchange_halo_2d_spatial_inplace(__or_local_cur, ctx.__or_spatial_layout, __or_output_rows, __or_output_cols, __or_input_cols, ctx.__or_followup_row_offset, ctx.__or_followup_col_offset, ctx.__or_spatial_halo_width, mpi_rank, ctx.mpi_size, MPI_DOUBLE);
    }
    if (__or_time_step > 0) {
    {
    ...
    for (int __or_block_step = 0; __or_block_step < __or_inner_steps; ++__or_block_step) {
        const int64_t __or_spatial_remaining_halo = __or_inner_steps - __or_block_step - 1;
        const int64_t __or_compute_row_begin = std::max<int64_t>(1, ctx.__or_spatial_layout.owned_row_offset - __or_spatial_remaining_halo);
        const int64_t __or_compute_row_end = std::min<int64_t>(__or_local_reader_rows - 1, ctx.__or_spatial_layout.owned_row_offset + __or_local_output_rows + __or_spatial_remaining_halo);
        const int64_t __or_compute_col_begin = std::max<int64_t>(1, ctx.__or_spatial_layout.owned_col_offset - __or_spatial_remaining_halo);
        const int64_t __or_compute_col_end = std::min<int64_t>(__or_local_reader_cols - 1, ctx.__or_spatial_layout.owned_col_offset + __or_local_output_cols + __or_spatial_remaining_halo);
        const int64_t __or_compute_rows = std::max<int64_t>(0, __or_compute_row_end - __or_compute_row_begin);
        const int64_t __or_compute_cols = std::max<int64_t>(0, __or_compute_col_end - __or_compute_col_begin);
        __or_kernel_item_count = dacpp::mpi::operator_resident::checked_mul_int64_or_abort(__or_compute_rows, __or_compute_cols, "[DACPP][MPI][OR] spatial temporal resident halo 2D kernel item count overflow");
        if (__or_kernel_item_count > 0) {
            ...
            q.submit([&](sycl::handler& h) {
                ...
                h.parallel_for<class waveEquation_mpi_dac_sycl_buffer_test_kernel_1>(sycl::range<1>(static_cast<std::size_t>(__or_kernel_item_count)), [=](sycl::id<1> idx) {
                    const int item_linear = static_cast<int>(idx[0]);
                    const int local_row = static_cast<int>(__or_compute_row_begin) + item_linear / static_cast<int>(__or_compute_cols);
                    const int local_col = static_cast<int>(__or_compute_col_begin) + item_linear % static_cast<int>(__or_compute_cols);
                    ...
                    dacpp::mpi::ResidentHaloView2D<const double> view_cur{__or_reader_data, static_cast<int>((local_row - 1) * static_cast<int>(__or_local_reader_cols) + local_col - 1), static_cast<int>(__or_local_reader_cols)};
                    dacpp::mpi::ContiguousView1D<const double> view_prev{__or_direct_reader_data, static_cast<int>(local_row * static_cast<int>(__or_local_reader_cols) + local_col)};
                    dacpp::mpi::ContiguousView1D<double> view_next{__or_writer_data, static_cast<int>(local_row * static_cast<int>(__or_local_reader_cols) + local_col)};
    ...
    // P11 spatial temporal-block=2 leaves the latest state in the resident reader buffer after spatial role rotation.
}
void __dacpp_mpi_or_waveEqShell_waveEq_0_materialize(__dacpp_mpi_or_waveEqShell_waveEq_0_ctx& ctx, dacpp::Matrix<double> & __or_arg0, dacpp::Matrix<double> & __or_arg1, dacpp::Matrix<double> & __or_arg2) {
    ...
    const auto __or_owned_next = dacpp::mpi::operator_resident::owned_slice_2d_spatial(ctx.__or_local_cur, ctx.__or_spatial_layout, ctx.__or_output_rows, ctx.__or_output_cols, 1, 1);
    ...
    {
        __dacpp_test::ScopedCommunicationTimer __dacpp_test_comm_timer;
        dacpp::mpi::operator_resident::gather_spatial_owned_to_root(__or_owned_next, __or_materialized_next, ctx.__or_output_rows, ctx.__or_output_cols, mpi_rank, ctx.mpi_size, MPI_DOUBLE);
    }
...
int main() {
    ...
    __dacpp_test::reset_communication_time();
...
for(int i = 0;false; i++) {
    ...
    __dacpp_test::print_summary("waveEquation.mpi.dac_sycl_buffer_test.cpp", mpi_rank, mpi_size, __dacpp_test_total_time);
