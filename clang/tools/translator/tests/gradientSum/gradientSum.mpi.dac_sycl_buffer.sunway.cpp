    ...
    if (__or_local_item_count > 0) {
        {
            ...
            q.submit([&](sycl::handler& h) {
                ...
                h.parallel_for<class __dacpp_sycl_kernel_gradientSum_translated_rowblock_0>(sycl::range<1>(static_cast<std::size_t>(__or_local_item_count)), [=](sycl::id<1> idx) {
...
int main() {
    ...
    const double __dacpp_test_e2e_time =
        std::chrono::duration<double>(__dacpp_test_e2e_end - __dacpp_test_e2e_start)
            .count();
    __dacpp_test::print_e2e_summary("gradientSum.mpi.dac_sycl_buffer.sunway.cpp",
                                    mpi_rank,
                                    mpi_size,
