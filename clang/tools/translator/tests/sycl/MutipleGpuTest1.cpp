...
void ODDEVEN_oddeven(const dacpp::Vector<int> & array, dacpp::Vector<int> & array_out) { 
    ...
    int recon_h_array[array_Size];
	int *recon_d_array=malloc_device<int>(array_Size,q);
	int *d_array=malloc_device<int>(array_Size,q);
	

    array.tensor2Array(recon_h_array);
    q.memcpy(recon_d_array, recon_h_array, array_Size*sizeof(int)).wait();
    array_tool.Reconstruct(d_array, recon_d_array,q);
	
	sycl::free(recon_d_array, q);
    ...
    if(Reduction_Split_Size > 1)
    {
        ...
        q.memcpy(d_array_out,reduction_array_out, array_outReduction_Size*sizeof(int)).wait();
    }


    // 归并结果返回
	int *d_myTensor2=malloc_device<int>(array_out_Size,q);
    array_out_tool.UpdateData(d_array_out,d_myTensor2,q);
	int* res = (int*)malloc(array_out_Size*sizeof(int));
	q.memcpy(res,d_myTensor2, array_out_Size*sizeof(int)).wait();
	array_out.array2Tensor(res);
    ...
    sycl::free(reduction_array_out, q);
    sycl::free(d_myTensor2, q);
