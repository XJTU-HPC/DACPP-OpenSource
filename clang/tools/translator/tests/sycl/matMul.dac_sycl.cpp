...
void matrixMultiply_shell(const dacpp::Tensor<int, 2> & matA, const dacpp::Tensor<int, 2> & matB, dacpp::Tensor<int, 2> & matC) { 
    ...
    for(int numDevice = 0; numDevice < numDevices; numDevice++){
        ...
        // 设备内存分配
        int *d_matA=malloc_device<int>(matA_Size,q[numDevice]);
        // 设备内存分配
        int *d_matB=malloc_device<int>(matB_Size,q[numDevice]);
        // 设备内存分配
        int *d_matC=malloc_device<int>(matC_Size,q[numDevice]);
        // 归约设备内存分配
        int *reduction_matC = malloc_device<int>(Reduction_Size,q[numDevice]);
    
        // 数据移动
        q[numDevice].memcpy(d_matA,r_matA,matA_Size*sizeof(int)).wait();
        // 数据移动
        q[numDevice].memcpy(d_matB,r_matB,matB_Size*sizeof(int)).wait();
	
        //工作项划分
        // printf("Item_Size is %d\n", Item_Size);
        sycl::range<3> global(1, 1, Item_Size);//全局任务
        sycl::range<3> local(1, 1, Item_Size/2);

        //队列提交命令组
        events.push_back(q[numDevice].submit([&](handler &h) {
            // 创建 stream 对象
            h.parallel_for(sycl::nd_range<3>(global * local, local),[=](sycl::nd_item<3> item) {
                const auto item_id = item.get_local_id(2) + numDevice * Item_Size/2;
                // 索引初始化
                // printf("Decice %d is runing\n", numDevice);
    ...
    for(int numDevice = 0; numDevice < numDevices; numDevice++){
        ...
        // 归并结果返回
        q[numDevice].memcpy(r_matC+8 *numDevice, d_matC+8 *numDevice, matC_Size/2*sizeof(int)).wait();
         // 将 d_matC 的数据复制回主机内存
         int* host_matC = (int*)malloc(sizeof(int) * Item_Size);
         q[numDevice].memcpy(host_matC, d_matC, Item_Size * sizeof(int)).wait();
