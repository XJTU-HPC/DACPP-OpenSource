...
void oddeven(int* array,int* array_out,sycl::accessor<int, 1, sycl::access::mode::read_write> info_array_acc, sycl::accessor<int, 1, sycl::access::mode::read_write> info_array_out_acc) 
{
    ...
    std::vector<sycl::event> events;
        ...
        std::vector<int*> d_arrays(numDevices);
        
        std::vector<int*> d_array_outs(numDevices);
        
        std::vector<int*> reduction_array_outs(numDevices);
        
    for(int numDevice = 0; numDevice < numDevices; numDevice++){
        // 设备内存分配
        
        // 设备内存分配
        int *d_array=malloc_device<int>(array_Size,q[numDevice]);
        // 设备内存分配
        int *d_array_out=malloc_device<int>(array_out_Size,q[numDevice]);
        // 归约设备内存分配
        int *reduction_array_out = malloc_device<int>(Reduction_Size,q[numDevice]);
        // 数据移动
        
        // 数据移动
        q[numDevice].memcpy(d_array,r_array,array_Size*sizeof(int)).wait();
        //内核执行
        
        //工作项划分
        sycl::range<3> local(1, 1, Item_Size/numDevices+1);
        ...
        auto info_partition_array_out_accessor = info_partition_array_out_buffer.get_access<sycl::access::mode::read_write>(h);
            h.parallel_for(sycl::nd_range<3>(global * local, local),[=](sycl::nd_item<3> item) {
                const auto item_id = item.get_local_id(2) + numDevice * Item_Size/numDevices;
                // 索引初始化
        ...
        //保存内存地址
        d_arrays[numDevice] = d_array;
        //保存内存地址
        d_array_outs[numDevice] = d_array_out;
        //保存内存地址
        reduction_array_outs[numDevice] = reduction_array_out;
    }
    for(int numDevice = 0; numDevice < numDevices; numDevice++){
        //获取内存地址
        
        //获取内存地址
        int *d_array = d_arrays[numDevice];
        //获取内存地址
        int *d_array_out = d_array_outs[numDevice];
        //获取内存地址
        int *reduction_array_out = reduction_array_outs[numDevice];
        // 归约
    ...
    if(Reduction_Split_Size > 1)
    {
        ...
        q[numDevice].memcpy(d_array_out,reduction_array_out, Reduction_Size*sizeof(int)).wait();
    }
    ...
    // 归并结果返回
    q[numDevice].memcpy(r_array_out + numDevice * array_out_Size / numDevices,d_array_out + numDevice * array_out_Size / numDevices, array_out_Size*sizeof(int)/numDevices).wait();
    // array_out_tool.UpdateData(r_array_out,array_out);
        // 内存释放
        
    sycl::free(d_array, q[numDevice]);
    sycl::free(d_array_out, q[numDevice]);
    }
    // 归并结果返回
    array_out_tool.UpdateData(r_array_out,array_out);
    }
