...
//     idx2.setDimId(1);
//     idx2.SetSplitSize(para_gene_tool.init_operetor_splitnumber(idx2,info_matA));

//     // 降维算子初始化
//     Index idx4 = Index("idx4");
//     idx4.setDimId(0);
//     idx4.SetSplitSize(para_gene_tool.init_operetor_splitnumber(idx4,info_matB));

//     // 降维算子初始化
//     Index idx3 = Index("idx3");
//     idx3.setDimId(1);
//     idx3.SetSplitSize(para_gene_tool.init_operetor_splitnumber(idx3,info_matB));
...
//     idx4.setDimId(0);
//     matB_Ops.push_back(idx4);

//     idx3.setDimId(1);
//     matB_Ops.push_back(idx3);
...
//         // 归约设备内存分配
//         int *d_matC = malloc_device<int>(matA_Device_Size[numDevice]*matB_Device_Size[numDevice]/ info_matB.dimLength[1]*matC_data_SplitSize,q[numDevice]);
//         // 归约设备内存分配
...
//         //工作项划分
//         sycl::range<3> local(1, 1, matA_Device_Size[numDevice]*matB_Device_Size[numDevice]/ info_matB.dimLength[1]);
...
//             const auto idx1_=(item_id/idx2.split_size/idx4.split_size+(0))%idx1.split_size;
//             const auto idx3_=(item_id/idx2.split_size/idx4.split_size+(0))%idx3.split_size;
//             const auto idx2_=(item_id/idx4.split_size+(0))%idx2.split_size;
//             const auto idx4_=(item_id+(0))%idx4.split_size;
//                 // 嵌入计算
                
//         matrixMultiply_calc(d_matA+(idx1_*SplitLength[0][0]+idx2_*SplitLength[0][1]),d_matB+(((idx4_+ Result_HaveMalloc/idx3.split_size)%idx4.split_size)*SplitLength[1][0]+((idx3_+ Result_HaveMalloc)%idx3.split_size)*SplitLength[1][1]),d_matC+(idx1_*SplitLength[2][0]+idx2_*SplitLength[2][1]+idx4_*SplitLength[2][2]),info_partition_matA_accessor,info_partition_matB_accessor,info_partition_matC_accessor);
//             });
//         }));
//         Result_HaveMalloc += matA_Device_Size[numDevice]*matB_Device_Size[numDevice]/ info_matB.dimLength[1];
...
//     // 归并结果返回
//     q[numDevice].memcpy(r_matC + matC_HaveMalloc, d_matC, matA_Device_Size[numDevice]*matB_Device_Size[numDevice]/ info_matB.dimLength[1]*matC_data_SplitSize*sizeof(int)).wait();
//     matC_HaveMalloc += matA_Device_Size[numDevice]*matB_Device_Size[numDevice]/ info_matB.dimLength[1]*matC_data_SplitSize;
...
// int main() {
//     // 初始化两个矩阵 A 和 B
//     std::vector<int> dataA{1, 2, 3, 4, 5, 6, 7, 8, 9};
//     dacpp::Matrix<int> matA({3, 3}, dataA);

//     std::vector<int> dataB{1, 5, 9, 13, 17, 2, 6, 10,3};
//     dacpp::Matrix<int> matB({3, 3}, dataB);


//     std::vector<int> dataC{1, 2, 3, 4, 5, 6, 7, 8, 9};
//     dacpp::Matrix<int> matC({3, 3}, dataC);
...
void MC_pi_mc_pi(const dacpp::Vector<int> & index, dacpp::Vector<int> & inside_circle) { 
    ...
    //生成设备内存分配大小
    int inside_circleReduction_Size = para_gene_tool.init_device_memory_size(info_inside_circle,Reduction_Ops);
    ...
    // 归约设备内存分配
    int *reduction_inside_circle = malloc_device<int>(inside_circleReduction_Size,q);
    // 数据关联计算
    ...
    if(Reduction_Split_Size > 1)
    {
        for(int i=0;i<inside_circleReduction_Size;i++) {
            q.submit([&](handler &h) {
    	        h.parallel_for(
                range<1>(Reduction_Split_Size),
        ...
        q.memcpy(d_inside_circle,reduction_inside_circle, inside_circleReduction_Size*sizeof(int)).wait();
    }
...
void random_walk_1D_shell_random_walk_1D_calc(const dacpp::Vector<int> & index, dacpp::Vector<int> & final_positions) { 
    ...
    //生成设备内存分配大小
    int final_positionsReduction_Size = para_gene_tool.init_device_memory_size(info_final_positions,Reduction_Ops);
    ...
    // 归约设备内存分配
    int *reduction_final_positions = malloc_device<int>(final_positionsReduction_Size,q);
    // 数据关联计算
    ...
    if(Reduction_Split_Size > 1)
    {
        for(int i=0;i<final_positionsReduction_Size;i++) {
            q.submit([&](handler &h) {
    	        h.parallel_for(
                range<1>(Reduction_Split_Size),
        ...
        q.memcpy(d_final_positions,reduction_final_positions, final_positionsReduction_Size*sizeof(int)).wait();
    }
...
void random_walk_2D_shell_random_walk_2D_calc(const dacpp::Vector<int> & index, dacpp::Vector<int> & final_positions_x, dacpp::Vector<int> & final_positions_y) { 
    ...
    //生成设备内存分配大小
    int final_positions_xReduction_Size = para_gene_tool.init_device_memory_size(info_final_positions_x,Reduction_Ops);

    //生成设备内存分配大小
    int final_positions_y_Size = para_gene_tool.init_device_memory_size(In_Ops,Out_Ops,info_final_positions_y);

    //生成设备内存分配大小
    int final_positions_yReduction_Size = para_gene_tool.init_device_memory_size(info_final_positions_y,Reduction_Ops);
    ...
    // 归约设备内存分配
    int *reduction_final_positions_x = malloc_device<int>(final_positions_xReduction_Size,q);
    // 设备内存分配
    int *d_final_positions_y=malloc_device<int>(final_positions_y_Size,q);
    // 归约设备内存分配
    int *reduction_final_positions_y = malloc_device<int>(final_positions_yReduction_Size,q);
    // 数据关联计算
    ...
    if(Reduction_Split_Size > 1)
    {
        for(int i=0;i<final_positions_yReduction_Size;i++) {
            q.submit([&](handler &h) {
    	        h.parallel_for(
                range<1>(Reduction_Split_Size),
        ...
        q.memcpy(d_final_positions_y,reduction_final_positions_y, final_positions_yReduction_Size*sizeof(int)).wait();
    }
