...
// Host-side dense matrix that mirrors the DACPP Tensor/Matrix data model used by
// the translated program. Element access goes through a shape-driven index
// computation, and host<->transport conversions are element-by-element, exactly
// like the translated code's Tensor::tensor2Array / Tensor::array2Tensor. This
// reproduces the real serialization cost the DAC path pays on the root process,
// so the comparison isolates parallel execution quality rather than rewarding a
// flat-buffer shortcut the DAC program cannot take.
struct HostMatrix {
...
int main(int argc, char** argv) {
    ...
    // Per-rank row partition metadata, shared by the initial scatter and the
    // final gather. This mirrors the DACPP source data semantics: the field is
    // one host-visible global matrix that is distributed to the ranks and then
    // reassembled on the root, not independently fabricated on each rank.
    ...
    // Build the full initial field on the root through the same Matrix data
    // abstraction the DACPP program uses, then serialize it to a flat transport
    // buffer (Tensor::tensor2Array) before scattering the owned rows into each
    // rank's halo interior.
    ...
    for (int step = 0; step < TIME_STEPS; ++step) {
...

        {
            ...
            // Copy the stencil output back into the persistent field, matching
            // the source statement matIn[i][j] = matOut[i-1][j-1]. The DACPP
            // followup-offset (1,1) is exactly this per-step copy-back; the
            // baseline intentionally does not take the double-buffer swap
            // shortcut, so it performs the same per-step data movement the DAC
            // path does.
            q.submit([&](sycl_compat::handler& h) {
                auto curr_acc =
    ...
    // Reassemble the full updated field on the root. The gathered flat buffer is
    // deserialized back into the Matrix abstraction (Tensor::array2Tensor), the
    // physical boundary conditions are applied through element accessors, and a
    // representative value is read out, matching the host-visible final matrix
    // the DACPP program materializes after the time loop.
    ...
    if (rank == 0) {
        ...
        // Consume the materialized matrix so the deserialization is observable
        // (mirrors the source-level matIn[0].print()).
        double checksum = 0.0;
        for (int j = 0; j < NY; ++j) {
