...
void stencilShell_stencil(const dacpp::Matrix<double> & matIn, dacpp::Matrix<double> & matOut) { 
    ...
    // 归约设备内存分配
    double *reduction_matOut = malloc_device<double>(matOutReduction_Size,q);
    // 数据关联计算
    
	    
	
    // 设备内存分配
    double *d_matIn=malloc_device<double>(matIn_Size,q);
    // 设备内存分配
    double *d_matOut=malloc_device<double>(matOut_Size,q);
    // 数据移动
	double* h_matIn = (double*)malloc(matIn_Size*sizeof(double));
	matIn.tensor2Array(h_matIn);
    q.memcpy(d_matIn,h_matIn,matIn_Size*sizeof(double)).wait();

    // 数据移动
	double* h_matOut = (double*)malloc(matOut_Size*sizeof(double));
	// matOut.tensor2Array(h_matOut);
    q.memset(d_matOut, 0, matOut_Size*sizeof(double)).wait();
    // 数据重组
	...
	double *r_matIn=malloc_device<double>(matIn_Size,q);
    matIn_tool.Reconstruct(r_matIn,d_matIn,q);
	...
	double *r_matOut=malloc_device<double>(matOut_Size,q);
    matOut_tool.Reconstruct(r_matOut,d_matOut,q);
    ...
    q.submit([&](handler &h) {
        ...
        h.parallel_for(sycl::nd_range<3>(global, local),[=](sycl::nd_item<3> item) {
            ...
            stencil(r_matIn+(sp1_*SplitLength[0][0]+sp2_*SplitLength[0][1]),r_matOut+(sp1_*SplitLength[1][0]+sp2_*SplitLength[1][1]),info_partition_matIn_accessor,info_partition_matOut_accessor);
        });
    }).wait();
        ...
        for(int i=0;i<matOutReduction_Size;i++) {
            q.submit([&](handler &h) {
                ...
                [=](id<1> idx,auto &reducer) {
                    reducer.combine(r_matOut[(i/Reduction_Split_Length)*Reduction_Split_Length*Reduction_Split_Size+i%Reduction_Split_Length+idx*Reduction_Split_Length]);
     	        });
         }).wait();
        }
        q.memcpy(r_matOut,reduction_matOut, matOutReduction_Size*sizeof(double)).wait();
    }


    // 归并结果返回
    matOut_tool.UpdateData(r_matOut,d_matOut,q);
	q.memcpy(h_matOut,d_matOut, matOut_Size*sizeof(double)).wait();
