...
inline void matrixMultiply_calc_mpi_local(dacpp::mpi::View1D<const int> vecA,
                                          dacpp::mpi::View1D<const int> vecB,
                                          dacpp::mpi::View1D<int> dotProduct) {
    for (int i = 0; i < 5; ++i) {
        dotProduct[0] += vecA[i] * vecB[i];
    }
}

void matrixMultiply_shell_matrixMultiply_calc(const dacpp::Matrix<int>& matA,
    ...
    for (int dim = 0; dim < matA.getDim(); ++dim) {
        pattern_vecA.data_info.dimLength.push_back(matA.getShape(dim));
    }
    Dac_Op pattern_vecA_op;
    pattern_vecA_op.setDimId(0);
    pattern_vecA_op.size = 1;
    pattern_vecA_op.stride = 1;
    pattern_vecA_op.SetSplitSize(matA.getShape(0));
    pattern_vecA.param_ops.push_back(pattern_vecA_op);
    ...
    if (binding_split_sizes.size() < pattern_vecA.bind_split_sizes.size()) {
        binding_split_sizes.resize(pattern_vecA.bind_split_sizes.size(), 1);
    }
    for (std::size_t bind_i = 0; bind_i < pattern_vecA.bind_split_sizes.size(); ++bind_i) {
        binding_split_sizes[bind_i] =
            std::max<int64_t>(binding_split_sizes[bind_i], pattern_vecA.bind_split_sizes[bind_i]);
    }
    ...
    for (int dim = 0; dim < matB.getDim(); ++dim) {
        pattern_vecB.data_info.dimLength.push_back(matB.getShape(dim));
    }
    Dac_Op pattern_vecB_op;
    pattern_vecB_op.setDimId(1);
    pattern_vecB_op.size = 1;
    pattern_vecB_op.stride = 1;
    pattern_vecB_op.SetSplitSize(matB.getShape(1));
    pattern_vecB.param_ops.push_back(pattern_vecB_op);
    ...
    if (binding_split_sizes.size() < pattern_vecB.bind_split_sizes.size()) {
        binding_split_sizes.resize(pattern_vecB.bind_split_sizes.size(), 1);
    }
    for (std::size_t bind_i = 0; bind_i < pattern_vecB.bind_split_sizes.size(); ++bind_i) {
        binding_split_sizes[bind_i] =
            std::max<int64_t>(binding_split_sizes[bind_i], pattern_vecB.bind_split_sizes[bind_i]);
    }
    ...
    for (int dim = 0; dim < matC.getDim(); ++dim) {
        pattern_dotProduct.data_info.dimLength.push_back(matC.getShape(dim));
    }
    Dac_Op pattern_dot_row;
    pattern_dot_row.setDimId(0);
    pattern_dot_row.size = 1;
    pattern_dot_row.stride = 1;
    pattern_dot_row.SetSplitSize(matC.getShape(0));
    pattern_dotProduct.param_ops.push_back(pattern_dot_row);
    ...
    Dac_Op pattern_dot_col;
    pattern_dot_col.setDimId(1);
    pattern_dot_col.size = 1;
    pattern_dot_col.stride = 1;
    pattern_dot_col.SetSplitSize(matC.getShape(1));
    pattern_dotProduct.param_ops.push_back(pattern_dot_col);
    ...
    if (binding_split_sizes.size() < pattern_dotProduct.bind_split_sizes.size()) {
        binding_split_sizes.resize(pattern_dotProduct.bind_split_sizes.size(), 1);
    }
    for (std::size_t bind_i = 0; bind_i < pattern_dotProduct.bind_split_sizes.size(); ++bind_i) {
        binding_split_sizes[bind_i] =
            std::max<int64_t>(binding_split_sizes[bind_i], pattern_dotProduct.bind_split_sizes[bind_i]);
    }

    int64_t total_items = 1;
    for (int64_t split_size : binding_split_sizes) {
    ...
    const int dot_partition_size =
        static_cast<int>(dacpp::mpi::partition_element_count(pattern_dotProduct));
    ...
    if (local_item_count > 0) {
        ...
        sycl::buffer<int, 1> buffer_dot(local_dotProduct.data(), sycl::range<1>(local_dotProduct.size()));
        sycl::buffer<int32_t, 1> slots_buffer_dot(slots_dotProduct.data(),
                                                   sycl::range<1>(slots_dotProduct.size()));
        ...
        q.submit([&](sycl::handler& h) {
            ...
            auto acc_dot = buffer_dot.get_access<sycl::access::mode::read_write>(h);
            auto slots_acc_dot = slots_buffer_dot.get_access<sycl::access::mode::read>(h);

            h.parallel_for(sycl::range<1>(static_cast<std::size_t>(local_item_count)),
                           [=](sycl::id<1> idx) {
                               const int item_linear = static_cast<int>(idx[0]);

                               auto* data_vecA =
                                   acc_vecA.template get_multi_ptr<sycl::access::decorated::no>().get();
                               auto* slot_vecA =
                                   slots_acc_vecA.template get_multi_ptr<sycl::access::decorated::no>().get();
                               dacpp::mpi::View1D<const int> view_vecA{
                                   data_vecA, slot_vecA, item_linear * vecA_partition_size};

                               auto* data_vecB =
                                   acc_vecB.template get_multi_ptr<sycl::access::decorated::no>().get();
                               auto* slot_vecB =
                                   slots_acc_vecB.template get_multi_ptr<sycl::access::decorated::no>().get();
                               dacpp::mpi::View1D<const int> view_vecB{
                                   data_vecB, slot_vecB, item_linear * vecB_partition_size};

                               auto* data_dot =
                                   acc_dot.template get_multi_ptr<sycl::access::decorated::no>().get();
                               auto* slot_dot =
                                   slots_acc_dot.template get_multi_ptr<sycl::access::decorated::no>().get();
                               dacpp::mpi::View1D<int> view_dot{
                                   data_dot, slot_dot, item_linear * dot_partition_size};

                               matrixMultiply_calc_mpi_local(view_vecA, view_vecB, view_dot);
                           });
        });
        q.wait();
    }
    ...
    const auto& writeback_globals_dotProduct =
        pack_dotProduct.writeback_globals.empty() ? pack_dotProduct.globals
                                                  : pack_dotProduct.writeback_globals;

    std::vector<int> synced_dotProduct;
    if (mpi_rank == 0) {
        std::vector<int> global_out_dotProduct;
        matC.tensor2Array(global_out_dotProduct);
        dacpp::mpi::apply_writeback_by_globals(writeback_dotProduct,
                                               writeback_globals_dotProduct,
                                               global_out_dotProduct);
        ...
        for (int peer = 1; peer < mpi_size; ++peer) {
            ...
            MPI_Recv(recv_globals.data(), recv_count, MPI_LONG_LONG, peer, 4002, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
            MPI_Recv(recv_values.data(), recv_count, MPI_INT, peer, 5002, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
        ...
        if (send_count > 0) {
            MPI_Send(const_cast<int64_t*>(writeback_globals_dotProduct.data()), send_count,
                     MPI_LONG_LONG, 0, 4002, MPI_COMM_WORLD);
...
int main() {
    ...
    std::vector<int> dataB{1, 5, 9, 13, 17, 2, 6, 10, 14, 18,
                           3, 7, 11, 15, 19, 4, 8, 12, 16, 20};
    dacpp::Matrix<int> matB({5, 4}, dataB);

    std::vector<int> dataC{1, 2, 3, 4, 5, 6, 7, 8,
